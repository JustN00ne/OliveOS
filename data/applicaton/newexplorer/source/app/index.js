...
// --- Search input content state logic ---
...
function checkInput() {
  ...
  if (input.value.trim() !== '') {
    ...
    if (searchIcon) searchIcon.classList.add('has-content');
  } else {
    input.classList.remove('has-content');
    input.classList.add('empty');
    if (searchIcon) searchIcon.classList.remove('has-content');
  }
}
if (input) {
...
// Responsive handler for action bar
function handleActionBarResponsive() {
  const actionBar = document.querySelector('.action_bar');
  if (!actionBar) return;
  // Buttons to hide on small screens
  const idsToHide = [
    'cut-file', 'copy-file', 'paste-file', 'rename-file', 'delete-file', 'sort-all', 'layout-all',
  ];
  // Buttons to show only on small screens
  const idsToShow = ['operations-more', 'overall-more'];
  // Always hide the 'more' buttons by default
  idsToShow.forEach(id => {
    const btn = document.getElementById(id);
    if (btn) btn.style.display = 'none';
  });
  // Calculate if the action bar is overflowing
  const isOverflowing = actionBar.scrollWidth > actionBar.clientWidth + 10;
  if (isOverflowing) {
...
window.addEventListener('resize', handleActionBarResponsive);
window.addEventListener('DOMContentLoaded', handleActionBarResponsive);
